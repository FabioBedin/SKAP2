...
a <- get_df_wide(dep) %>%
  select(c(name, Protein.IDs, Protein.names, imputed, starts_with(conditions), -ends_with(c("CI.R", "CI.L")))) %>% 
  dplyr::rename_with(., ~ gsub(pattern = paste0(test, "_"), replacement = "", .), starts_with(test)) %>% 
  dplyr::rename_with(., ~ gsub(pattern = "^", replacement = "LFQ_intensity_", .), starts_with(conditions)) %>%
  dplyr::rename(Fold.Change = diff, Gene.name = name) %>% 
  mutate(significant = if_else(significant, "+", "")) %>% 
...
if(leng_dep != 0){
  ...
  if(n_edge != 0){
    ...
    file.rename(from = here::here("docs/assets/network_FC_pvalue.png"), to = here::here(paste0("docs/assets/network_FC_pvalue_", test, ".png")))
  }else{
  print("NOT enough data.")
  }
...
if(enrich_map){
  ...
  file.rename(from = here::here("docs/assets/network_enrichmentmap.png"), to = here::here(paste0("docs/assets/network_enrichmentmap_", test, ".png")))
